package com.example.android_project.data.api.nominatim;
...
public interface NominatimService {
    ...
    @Headers({"User-Agent: Android-Project"})
    @GET("/reverse?zoom=10&format=jsonv2&countrycodes=fr")
    Call<NominatimReverse> getCityByReverse(@Query("lat") String lat, @Query("lon") String lon);

    @Headers({"User-Agent: Android-Project"})
    @GET("/reverse?zoom=10&format=jsonv2&countrycodes=fr")
    Call<NominatimReverse> getCountyByReverse(@Query("lat") String lat, @Query("lon") String lon);
